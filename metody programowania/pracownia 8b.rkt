...
;;introducing lazy-lets:

(define (let-lazy? expr)
  (if (tagged-tuple? 'lazy 3 expr)
      (let-def? (second expr))
      false))

(define (let-lazy-expr e)
  (caddr e))

(define (let-lazy-def-expr e)
  (cadr e))

(define (let-lazy-def e)
  (cadr e))

(define (let-lazy-def-var e)
  (car e))
...
(define (let-lazy-def-cons x e)
  (list x e))

(define (let-lazy-cons def e)
  (list 'lazy def e))
...
(define (expr? t)
  (or (const? t)
      ...
      (var? t)
      (and (let-lazy? t)
           (expr? (let-lazy-expr t))
           (expr? (let-lazy-def-expr (let-lazy-def t))))
      (and (app? t)
           (expr? (app-proc t))
           (andmap expr? (app-args t)))))


;; environments (changed a bit)

(define empty-env
  null)


;;changed second clause in cond
(define (find-in-env x env)
  (cond [(null? env) (error "undefined variable" x)]
        [(eq? x (second (car env)))
         (cond [(eq? (first (car env)) 'lazy) ((third (car env)))]
               [else   (third (car env))])]
        [else (find-in-env x (cdr env))]))
 
;; attached name  = tag
(define (add-to-env name x v env)
  ...
  (cond [(const? e)
   ...
   ;;changed the procedure env-for-let, so let must change now too     
        ...
        [(let-lazy? e) (eval-env (let-expr e)
                                 (env-for-let 'lazy (let-def e) env))]
        [(app? e)
         (apply-closure
...
(define (env-for-let name def env)
  (add-to-env name
              (let-def-var def)
              (cond [(eq? name 'lazy) (lambda () (eval-env (let-def-expr def) env))]
                    [(eq? name 'common) (eval-env (let-def-expr def) env)])
                    env))
...
;;rozwiązanie powyższego zadania zostało konsultowane z Sebastianem Kondraciuk,
;;zaś przygotowane z pomocą Michała Symoszyn i Julii Nowogrodzkiej