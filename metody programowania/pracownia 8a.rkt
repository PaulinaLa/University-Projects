...
(define (ewaluator wyr)

  (eval `((lambda-rec (arith expr)
                        ...
                        (let (binop-op (lambda (e) (car e)))
                          (let (binop-left (lambda (e) (car (cdr e))))
                            (let (binop-rght (lambda (e) (car (cdr (cdr e)))))
                              (if (my-binop? expr) (cond [(eq? (binop-op expr) (quote add)) (+ (arith (binop-left expr)) (arith (binop-rght expr)))]
                                                         [(eq? (binop-op expr) (quote sub)) (- (arith (binop-left expr)) (arith (binop-rght expr)))]
                                                         [(eq? (binop-op expr) (quote mul)) (* (arith (binop-left expr)) (arith (binop-rght expr)))]
                                                         [(eq? (binop-op expr) (quote div)) (/ (arith (binop-left expr)) (arith (binop-rght expr)))])
                                  expr))))))
          ,wyr)))
...
(define (test)
  (and
   (= 12
      (ewaluator '(cons (quote add)
                        (cons  (cons (quote add) (cons 2 (cons 4 null)))
                               (cons  (cons (quote add) (cons 2 (cons 4 null))) null)))))
   (= 6
      (ewaluator '(cons (quote mul)
                        (cons 2 (cons 3 null)))))
   (= 0
      (ewaluator '(cons (quote sub)
                        (cons (cons (quote add) (cons 2 (cons 3 null)))
                              (cons 5 null)))))
   (= 10
      (ewaluator '(cons (quote div)
                        (cons (cons (quote mul)
                                    (cons (cons (quote add) (cons 4 (cons 6 null)))
                                          (cons 10 null)))
                              (cons 10 null)))))
   (= 8
      (ewaluator '(cons (quote add)
                        (cons (cons (quote add)
                                    (cons (cons (quote add)
                                                (cons 1 (cons 1 null)))
                                          (cons (cons (quote add)
                                                      (cons 1 (cons 1 null))) null)))
                              (cons (cons (quote add)
                                          (cons (cons (quote add)
                                                      (cons 1 (cons 1 null)))
                                                (cons (cons (quote add)
                                                      (cons 1 (cons 1 null))) null))) null)))))
   (= 1
      (ewaluator '(cons (quote sub)
                        (cons (cons (quote add)
                                    (cons 4 (cons 6 null)))
                              (cons (cons (quote div)
                                          (cons (cons (quote mul)
                                                      (cons 3 (cons 9 null)))
                                                (cons (cons (quote add)
                                                      (cons 2 (cons 1 null))) null))) null)))))))
             
(test)



;;powyższe zadanie powstało we współpracy z Michałem Symoszyn i Julią Nowogrodzką.
