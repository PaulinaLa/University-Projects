...
(define (solve-ax+by=1 a b)
  (define (iter p1 p2 s1 s2 r1 r2)
  (if (not (= 0 (remainder r1 r2)))
      (let ((q (floor (/ r1 r2))))
        (define (op x y) (- x (* y q)))
        (iter p2 (op p1 p2) s2 (op s1 s2) r2 (remainder r1 r2)))
...
(define (RSA-unconvert-list intlist key)
  (let ((n (key-modulus key)))
    ...
    (define (unconvert l sum)
       (if (null? l)
        null
        ;; odwracam operacje, które zaimplementowano w convert
        (let ((x (modulo (+ sum (RSA-transform (car l) key)) n)))
...
(define (make-signed-message signature message) (cons signature message))

(define (message make-signed-message) (cdr make-signed-message))

(define (signature make-signed-message) (car make-signed-message))
...
(define (encrypt-and-sign message recipient-public-key sender-private-key)
  ...
  (define caption
    (car (RSA-convert-list (list (compress cyphertext)) sender-private-key)))
  (make-signed-message caption cyphertext))


(define (authenticate-and-decrypt cyphertext recipient-private-key sender-public-key)
  (define text
    (RSA-unconvert-list (message cyphertext) recipient-private-key))
  (define real-signature
    (compress (message cyphertext)))
  (define predicted-signature
    (RSA-unconvert-list (list (signature cyphertext)) sender-public-key))
    (if (= real-signature (car predicted-signature))
        (intlist->string text)
...
;;; Przy tworzeniu procedury "solve-ax+by=1" posiłkowałam się filmem:
;;  https://www.youtube.com/watch?v=FjliV5u2IVw
;;  Testy działania napisanych procedur:

(RSA-encrypt "metody programowania sa super" (car test-key-pair1 ))

(RSA-decrypt '(675153482
  287248598
  267191018
  353356054
  100500942
  439005250
  544802161
  386696922) (cdr test-key-pair1))

(RSA-encrypt "testujemy zadanie na metody z rsa" (car test-key-pair2 ))

(RSA-decrypt '(376594566
  86251510
  274970093
  118313414
  486505651
  ...
  346231012
  220915507
  221717973) (cdr test-key-pair2))

(encrypt-and-sign "test zgodnosci podpisow" (car test-key-pair1) (cdr test-key-pair2))

;;zgodne podpisy, oczekiwany wynik;to treść wiadomośći

(authenticate-and-decrypt '(346627122
  185458897
  24462201
  288659721
  314206918
  179376406
  747524475) (cdr test-key-pair1) (car test-key-pair2))

;; wywołanie z błędnymi kluczami:
...
(authenticate-and-decrypt '(346627122
  ...
  314206918
  179376406
  747524475) (cdr test-key-pair1) (cdr test-key-pair2))